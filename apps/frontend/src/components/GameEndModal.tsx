...
import { GameResult, Result } from "../Screens/GamingPage";
...
export const GameEndModal:React.FC<ModlaProps> = ({
    blackPlayer,
    whitePlayer,
    gameResult
}) => {
     ...
     const PlayerDisplay=({player,gameResult,isWhite}:{
        player?:{
            id:string,
            name:string
        };
       ...
       const borderColor=gameResult===(isWhite ? Result.WHITE_WINS : Result.BLACK_WINS) ?
          ...
          <div className="relative rounded-lg shadow-lg bg-gray-800 w-96">
            <div className="px-6 py-8 items-center self-center m-auto">
              ...
              <div className="flex flex-row justify-between items-center bg-gray-700 rounded-lg px-4 py-6">
                ...
                <PlayerDisplay
                 isWhite={false} player={blackPlayer} gameResult={gameResult.result} />
              </div>
            </div>
